...
unsigned int read_code(FILE* fd)
{
	...
	read_count = fread(bytes, sizeof(unsigned char), 3, fd);
	//Error check
	if (ferror(fd))
	{
		perror("fread");
	...
	//Manipulate bit boundaries from a 12-bit system to align with read
	final_code = ((bytes[0] & 0xff) << 4);
	final_code = (final_code | (bytes[1] & 0x0f0) >> 4);
	pword = ((bytes[1] & 0xf) << 8); 
	pword = (pword | (bytes[2]));
	return final_code;
}
...
void uncompress(char *in_file_name, char *out_file_name)
{
	...
	// Output CurrentChar
	if(fwrite(cur_str, sizeof(unsigned char), cur_str_length, out_fd) != cur_str_length)
	{
		perror("write");
		exit(1);
	}
